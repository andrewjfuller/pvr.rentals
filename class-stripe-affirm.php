...
class EH_Affirm extends WC_Payment_Gateway {
    ...
    public function __construct() { 
        ...
        $this->method_title       = __( 'Affirm', 'eh-stripe-gateway' );

        $this->method_description = sprintf(__( 'Affirm is a ‘Buy Now, Pay Later’ method that enables customers in the US to pay in installments. %1$s[Preview] %2$s', 'eh-stripe-gateway' ), '<a  class="thickbox" href="'.EH_STRIPE_MAIN_URL_PATH . 'assets/img/affirm-preview.png?TB_iframe=true&width=100&height=100">', '</a>');
        $this->supports = array(
            'products',
            'refunds',
        );
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            ...
            'eh_stripe_affirm_title'         => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_affirm_title']) ? $stripe_settings['eh_stripe_affirm_title'] : __('Affirm', 'eh-stripe-gateway'),
                'desc_tip'    => true,
            ),
            'eh_stripe_affirm_description'     => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_affirm_description']) ? $stripe_settings['eh_stripe_affirm_description'] : __('Accept payments using Affirm.', 'eh-stripe-gateway'),
                'desc_tip'    => true
            ),
            ...
            'eh_stripe_affirm_order_button'    => array(
                ...
                'default'     => isset($stripe_settings['eh_stripe_affirm_order_button']) ? $stripe_settings['eh_stripe_affirm_order_button'] :__('Pay via Affirm', 'eh-stripe-gateway'),
                'desc_tip'    => true
            )
        );
    ...
    public function get_icon() {
        ...
        $icon = '<img src="' . esc_url(WC_HTTPS::force_https_url(EH_STRIPE_MAIN_URL_PATH . 'assets/img/affirm.png')) . '" width="54" alt="Affirm" title="Affirm" ' . $style . ' />';
    ...
    public function process_payment($order_id) { 
        ...
        try{ 
            ...
            if (!empty($customer) && isset($customer->id)) {
                ...
                if(! empty($intent)){
                    ...
                    if ( $intent->status === 'succeeded' ) {
                        wc_add_notice(__('An error has occurred internally, due to which you are not redirected to the order received page. Please contact support for more assistance.', 'eh-stripe-gateway'), 'error');
            ...
            $order->update_status( 'failed', sprintf( __( 'Affirm payment failed: %s', 'eh-stripe-gateway' ),$e->getMessage() ) );
    ...
    public function eh_process_payment_response( $response, $order = null ) {
            ...
            if (isset($charge_response->paid) && $charge_response->paid == true) {
                ...
                if(isset($charge_response->captured) && $charge_response->captured == true){
                    if($order->status != 'processing'){
                        ...
                        $order->add_order_note( __('Payment Status : ', 'eh-stripe-gateway') . ucfirst($charge_status) .' [ ' . $order_time . ' ] . ' . __('Source : ', 'eh-stripe-gateway') . $payment_method_tye . '. ' . __('Charge Status :', 'eh-stripe-gateway') . $captured . (is_null($charge_response->balance_transaction) ? '' :'. Transaction ID : ' . $charge_response->balance_transaction) );
        ...
        elseif($response->status == 'processing' || $response->status == 'pending'){
            $order->update_status( 'on-hold', __( 'Waiting for the payment to succeed or fail.', 'eh-stripe-gateway' ) );

        }         
        elseif($response->status == 'requires_capture'){
            $order->update_status( 'on-hold', __( 'Payment is authorized and requires a capture.', 'eh-stripe-gateway' ) );
            $order->add_order_note( __('Payment Status : ', 'eh-stripe-gateway') . ucfirst($charge_status) .' [ ' . $order_time . ' ] . ' . __('Source : ', 'eh-stripe-gateway') . $payment_method_tye . '. ' . __('Charge Status :', 'eh-stripe-gateway') . $captured . (is_null($charge_response->balance_transaction) ? '' :'. Transaction ID : ' . $charge_response->balance_transaction) );
