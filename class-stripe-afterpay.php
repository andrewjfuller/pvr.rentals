...
class EH_Afterpay extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title       = __( 'Afterpay', 'eh-stripe-gateway' );

        $this->method_description =  __( 'Accepts payments via Afterpay (also known as Clearpay in the UK) that provides a ‘Buy Now, Pay Later’ option for customers. ' . '<a  class="thickbox" href="'.EH_STRIPE_MAIN_URL_PATH . 'assets/img/afterpay-preview.png?TB_iframe=true&width=100&height=100">[Preview] </a><ul></i>Supports currencies <b>USD, CAD, GBP, AUD, NZD, EUR</b></li></ul><p> <a target="_blank" href="https://www.webtoffee.com/accept-afterpay-using-woocommerce-stripe-payment-gateway/"> Read Documentation </a></p>', 'eh-stripe-gateway' );
        $this->supports = array(
            'products',
            'refunds',
        );
        ...
        $this->order_button_text       = __($this->eh_order_button, 'eh-stripe-gateway');
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            ...
            'enabled'                       => array(
                'title'       => __('Afterpay','eh-stripe-gateway'),
                'label'       => __('Enable','eh-stripe-gateway'),
                'type'        => 'checkbox',
                'default'     => isset($stripe_settings['eh_stripe_afterpay']) ? $stripe_settings['eh_stripe_afterpay'] : 'no',
                'desc_tip'    => __('Enables to accept payments using Afterpay.','eh-stripe-gateway'),
            ),

            'eh_stripe_afterpay_title'         => array(
                'title'       => __('Title','eh-stripe-gateway'),
                'type'        => 'text',
                'description' =>  __('Input title for the payment gateway displayed at the checkout.', 'eh-stripe-gateway'),
                'default'     =>isset($stripe_settings['eh_stripe_afterpay_title']) ? $stripe_settings['eh_stripe_afterpay_title'] : __('Afterpay', 'eh-stripe-gateway'),
                'desc_tip'    => true,
            ),
            'eh_stripe_afterpay_description'     => array(
                ...
                'type'        => 'textarea',
                'css'         => 'width:25em',
                'description' => __('Input texts for the payment gateway displayed at the checkout.', 'eh-stripe-gateway'),
                'default'     =>isset($stripe_settings['eh_stripe_afterpay_description']) ? $stripe_settings['eh_stripe_afterpay_description'] : __('Accept payments using Afterpay.', 'eh-stripe-gateway'),
                'desc_tip'    => true
            ),
            ...
            'eh_stripe_afterpay_order_button'    => array(
                ...
                'type'        => 'text',
                'description' => __('Input a text that will appear on the order button to place order at the checkout.', 'eh-stripe-gateway'),
                'default'     => isset($stripe_settings['eh_stripe_afterpay_order_button']) ? $stripe_settings['eh_stripe_afterpay_order_button'] :__('Pay via Afterpay', 'eh-stripe-gateway'),
                'desc_tip'    => true
            )
        );   
    }
    ...
    public function get_icon() {
        ...
        $icon .= '<img src="' . WC_HTTPS::force_https_url(EH_STRIPE_MAIN_URL_PATH . 'assets/img/afterpay.png') . '" width="82" alt="Afterpay" title="Afterpay" ' . $style . ' />';
    ...
    public function process_payment($order_id) { 
        ...
        try{ 
            ...
            if (empty($payment_method)) {
                throw new Exception(__('Unable to process this payment, please try again.', 'eh-stripe-gateway' ));
                
            }
            $currency =  $order->get_currency();
            $amount = EH_Stripe_Payment::get_stripe_amount(((WC()->version < '2.7.0') ? $order->order_total : $order->get_total())) ;

             $customer = $this->create_stripe_customer($order_id, ((WC()->version < '2.7.0') ? $order->billing_email : $order->get_billing_email()), ((WC()->version < '2.7.0') ? $order->billing_first_name . " " . $order->billing_last_name : $order->get_billing_first_name() . " " . $order->get_billing_last_name()));
            ...
            if (!empty($customer) && isset($customer->id)) {
                ...
                if (isset($intent->status) && ( $intent->status == 'requires_action' ) &&
                    ...
                    EH_Stripe_Payment::save_stripe_fee($intent, $order_id, false, false);
            ...
            $order->update_status( 'failed', sprintf( __( 'Afterpay payment failed: %s', 'eh-stripe-gateway' ),$e->getMessage() ) );
    ...
    public function save_payment_intent_to_order( $order, $intent ) {
        ...
        if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
            update_post_meta( $order_id, 'eh_afterpay_intent_id', $intent->id );
        } else {
            $order->update_meta_data( 'eh_afterpay_intent_id', $intent->id );
        }
     ...
     */
    public function create_stripe_customer( $order_id, $user_email = false, $name = false) {
    ...
    public function get_charge_details( $wc_order, $client, $currency, $amount, $payment_method) {
        ...
        $charge = array(
            ...
            'description' => sprintf(__('%1$s Order #%2$s', 'eh-stripe-gateway'), wp_specialchars_decode( __(get_bloginfo( 'name' ), 'eh-stripe-gateway'), ENT_QUOTES ), $wc_order->get_order_number() ),
        );
        ...
        if($show_items_details){
            ...
            $charge['description']= sprintf(__('%1$s %2$s Order #%3$s', 'eh-stripe-gateway'), $charge['metadata']['Products'], wp_specialchars_decode( __(get_bloginfo( 'name' ), 'eh-stripe-gateway'), ENT_QUOTES ), $wc_order->get_order_number() );
        }
        $charge['confirm'] =  true ;
        ...
        if ( (WC()->cart->needs_shipping()) ) {
            ...
            $charge['shipping'] = array(
                'address' => array(
                    'line1' => (WC()->version < '2.7.0') ? $wc_order->billing_address_1 : $wc_order->get_billing_address_1(),
                    'line2' => (WC()->version < '2.7.0') ? $wc_order->billing_address_2 : $wc_order->get_billing_address_2(),
                    'city' => (WC()->version < '2.7.0') ? $wc_order->billing_city : $wc_order->get_billing_city(),
                    'state' => (WC()->version < '2.7.0') ? $wc_order->billing_state : $wc_order->get_billing_state(),
                    'country' => (WC()->version < '2.7.0') ? $wc_order->billing_country : $wc_order->get_billing_country(),
                    'postal_code' => (WC()->version < '2.7.0') ? $wc_order->billing_postcode : $wc_order->get_billing_postcode()
                ),
                'name' => ((WC()->version < '2.7.0') ? $wc_order->shipping_first_name : $wc_order->get_billing_first_name()) . ' ' . ((WC()->version < '2.7.0') ? $wc_order->billing_last_name : $wc_order->get_billing_last_name()),
                'phone' => (WC()->version < '2.7.0') ? $wc_order->billing_phone : $wc_order->get_billing_phone(),
            );            
        }
    ...
    public function eh_make_charge_params( $order, $customer = null ) {
        ...
        $post_data['description']        =  sprintf( __( '%1$s - Order %2$s', 'eh-stripe-gateway' ), wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ), $order->get_order_number() );
        ...
        $post_data['metadata']  = array(
            __( 'customer_name', 'eh-stripe-gateway' ) => sanitize_text_field( $billing_first_name ) . ' ' . sanitize_text_field( $billing_last_name ),
            __( 'customer_email', 'eh-stripe-gateway' ) => sanitize_email( $billing_email ),
            ...
            if ('Captured' === $status) {
                ...
                try {
                    ...
                    if ($refund_response) {
                        ...
                        $order->add_order_note(__('Reason : ', 'eh-stripe-gateway') . $reason . '.<br>' . __('Amount : ', 'eh-stripe-gateway') . get_woocommerce_currency_symbol() . $amount . '.<br>' . __('Status : refunded ', 'eh-stripe-gateway') . ' [ ' . $refund_time . ' ] ' . (is_null($data['transaction_id']) ? '' : '<br>' . __('Transaction ID : ', 'eh-stripe-gateway') . $data['transaction_id']));
                        ...
                        $order->add_order_note(__('Reason : ', 'eh-stripe-gateway') . $reason . '.<br>' . __('Amount : ', 'eh-stripe-gateway') . get_woocommerce_currency_symbol() . $amount . '.<br>' . __(' Status : Failed ', 'eh-stripe-gateway'));
                    ...
                    $order->add_order_note(__('Reason : ', 'eh-stripe-gateway') . $reason . '.<br>' . __('Amount : ', 'eh-stripe-gateway') . get_woocommerce_currency_symbol() . $amount . '.<br>' . __('Status : ', 'eh-stripe-gateway') . $oops['error']['message']);
    ...
    public function get_payment_intent_from_order( $order ) {
        ...
        if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
            $intent_id = get_post_meta( $order_id, 'eh_afterpay_intent_id', true );
        } else {
            $intent_id = $order->get_meta( 'eh_afterpay_intent_id' );
        }
    ...
    public function eh_afterpay_callback_handler() {
        ...
        if (isset($_REQUEST['payment_intent']) && !empty($_REQUEST['payment_intent'])) {
            ...
            $intent_result = \Stripe\PaymentIntent::retrieve( array('id' => $intent_id, 'expand' => array('charges.data.balance_transaction') ) );

            if (!empty($intent_result)) {
                EH_Stripe_Payment::save_stripe_fee($intent_result, $order_id, false);
                ...
                if ($order) {
                $order->update_status( 'failed', __( 'Stripe payment failed', 'eh-stripe-gateway' ) );
                }
                
                wc_add_notice( __( 'Unable to process this payment.', 'eh-stripe-gateway' ), 'error' );
    ...
    public function eh_process_payment_response( $response, $order = null ) {
        ...
        if($response->status == 'succeeded' || $response->status == 'requires_capture'){
            if ($charge_response->paid == true) {
                ...
                if('processing' !== $order->status && $charge_response->captured == true){
                    $order->payment_complete( $charge_response->id );
                }
                if ('on-hold' !== $order->status && !$charge_response->captured) {
                    $order->update_status('on-hold');
                }
                $order->add_order_note( __('Payment Status : ', 'eh-stripe-gateway') . ucfirst($charge_response->status) .' [ ' . $order_time . ' ] . ' . __('Source : ', 'eh-stripe-gateway') . $charge_response->payment_method_details->type . '. ' . __('Charge Status :', 'eh-stripe-gateway') . $captured . (is_null($charge_response->balance_transaction) ? '' :'. Transaction ID : ' . (isset($charge_response->balance_transaction->id) ? $charge_response->balance_transaction->id : $charge_response->balance_transaction)));
                WC()->cart->empty_cart();
